...
class CianParser:
    ...
    def extract_metro_station(self):
        ...
        try:
            # Ищем элемент метро с селекторами ЦИАН
            metro_selectors = [
            ...
            for selector in metro_selectors:
                ...
                for elem in elements:
                    ...
                    if text and 'метро' not in text.lower() and 'мин' not in text.lower():
            ...
            if address:
                # Попробуем найти станции метро по известным названиям
                stations = [
            ...
            page_text = self.driver.page_source
            metro_match = re.search(r'"underground":\s*"([^"]+)"', page_text)
            if metro_match:
                return metro_match.group(1)
    ...
    def extract_metro_time(self):
        ...
        try:
            # Ищем элементы с информацией о метро
            ...
            # Ищем в тексте страницы
            page_text = self.driver.page_source
            match = re.search(r'"undergroundTime":\s*"([^"]+)"', page_text)
            if match:
                return match.group(1)
            
            # Ищем в описании (но не парсим само описание)
            elements = self.driver.find_elements(By.CSS_SELECTOR, '[data-name="Description"]')
            for elem in elements:
                text = elem.text[:200]  # Смотрим только начало
                match = re.search(r'(\d+)\s*мин\s*(?:до|пешком)', text)
                if match:
                    return f"{match.group(1)} мин"
    ...
    def show_stats(self):
        ...
        try:
            ...
            print("\n🚇 Объявления с информацией о метро:")
            self.cursor.execute(f"""
                SELECT COUNT(*) FROM {self.table_name} 
                WHERE metro_station IS NOT NULL OR metro_time IS NOT NULL
            """)
            metro_count = self.cursor.fetchone()[0]
            print(f"  С информацией о метро: {metro_count} из {total}")
            
            print("\n📅 Последние добавленные:")
            self.cursor.execute(f"""
                SELECT cian_id, title, price, metro_station, metro_time, floor_current, floor_total
                FROM {self.table_name} 
            ...
            for r in recent:
                ...
                print(f"  ID: {r[0]}, Цена: {r[2]:,} ₽{metro_info}{floor_info}")
    ...
    def run_parsing(self):
        ...
        for url in urls[:2]:  # Теперь парсим обе страницы
            print(f"\n🌐 Парсим: {url}")
            ...
            for i, offer in enumerate(offers[:8]):  # Увеличили лимит
                ...
                time.sleep(1.5)  # Уменьшили паузу
    ...
    def run(self):
        ...
        if use_db:
            ...
            print(f"   SELECT * FROM {self.table_name} WHERE metro_station IS NOT NULL ORDER BY created_at DESC;")
            print(f"   SELECT COUNT(*) FROM {self.table_name} WHERE metro_station IS NOT NULL;")
