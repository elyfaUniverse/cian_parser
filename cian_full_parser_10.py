...
class CianParser:
    ...
    def parse_offer(self, offer):
        ...
        try:
            ...
            data = {
                ...
                'building_type': self.extract_building_type_improved(),  # Используем улучшенный метод
                ...
                'metro_station': self.extract_metro_station_improved(),  # Используем улучшенный метод
                'metro_time': self.extract_metro_time_improved(),  # Используем улучшенный метод
    ...
    def extract_building_type_improved(self):
        """Извлечь тип дома - УЛУЧШЕННЫЙ МЕТОД"""
        try:
            # 1. Сначала пробуем найти в блоке "Дополнительная информация"
            info_selectors = [
            ...
            for selector in info_selectors:
                try:
                    ...
                    for elem in elements:
                        ...
                        # Проверяем все возможные типы домов
                        if 'панельный' in text or 'панель' in text:
                            return 'панельный'
                        elif 'кирпичный' in text or 'кирпич' in text:
                            return 'кирпичный'
                        elif 'монолитный' in text or 'монолит' in text:
                            return 'монолитный'
                        elif 'блочный' in text or 'блок' in text:
                            return 'блочный'
                        elif 'деревянный' in text or 'дерево' in text:
                            return 'деревянный'
                        elif 'сталинский' in text or 'сталинка' in text:
                            return 'сталинский'
                        elif 'хрущевский' in text or 'хрущевка' in text:
                            return 'хрущевский'
                        elif 'брежневский' in text or 'брежневка' in text:
                            return 'брежневский'
                except:
                    continue
            
            # 2. Ищем в характеристиках
            elements = self.driver.find_elements(By.CSS_SELECTOR, '[data-name="Features"]')
            for elem in elements:
                text = elem.text.lower()
                
                # Проверяем строки с типом дома
                lines = text.split('\n')
                for line in lines:
                    line = line.strip()
            ...
            # 3. Ищем в HTML-коде страницы
            page_text = self.driver.page_source.lower()
            
            # Ищем в JSON-LD данных
            json_ld_match = re.search(r'"houseType":\s*"([^"]+)"', page_text, re.IGNORECASE)
            if json_ld_match:
                house_type = json_ld_match.group(1).lower()
            ...
            # 4. Ищем по серии дома (это надежный признак)
            page_text_full = self.driver.page_source
            series_patterns = [
                r'1Лг-606', r'1-515', r'1-511', r'П-44', r'П-3', r'П-55', r'П-46',
                r'1-528', r'1-528КП', r'1-528КП-80', r'П-30', r'П-43'
            ]
            ...
            # 5. Ищем в описании (первые 500 символов)
            try:
                description = self.driver.find_element(By.CSS_SELECTOR, '[data-name="Description"]').text[:500].lower()
                if 'панельный' in description:
                    return 'панельный'
                elif 'кирпичный' in description:
                    return 'кирпичный'
                elif 'монолитный' in description:
                    return 'монолитный'
                elif 'блочный' in description:
                    return 'блочный'
            except:
                pass
            
            # 6. Ищем по ключевым словам в любом месте страницы
            if 'панельный' in page_text or 'панель' in page_text:
                return 'панельный'
            elif 'кирпичный' in page_text or 'кирпич' in page_text:
                return 'кирпичный'
            elif 'монолитный' in page_text or 'монолит' in page_text:
                return 'монолитный'
            elif 'блочный' in page_text or 'блок' in page_text:
                return 'блочный'
            elif 'деревянный' in page_text or 'дерево' in page_text:
                return 'деревянный'
    ...
    def extract_metro_station_improved(self):
        ...
        try:
                ...
                try:
                    ...
                    for elem in elements:
                        ...
                        if text and len(text) < 50:
                            ...
                            if station and 'метро' not in station.lower():
                                return station
                except:
                    continue
            
            # 2. Ищем в блоке "Ближайшее метро"
            try:
                metro_section = self.driver.find_element(By.XPATH, "//*[contains(text(), 'Ближайшее метро') or contains(text(), 'Станция метро')]")
                parent = metro_section.find_element(By.XPATH, "..")
                text = parent.text
                ...
                for line in lines:
                    ...
                    if line and 'метро' not in line.lower() and 'ближайшее' not in line.lower():
                        ...
                        station = re.sub(r'\d+\s*мин', '', station).strip()
                        if station:
                            return station
            except:
                pass
            
            # 3. Ищем в JSON-LD данных
            page_text = self.driver.page_source
            json_ld_match = re.search(r'"addressLocality":\s*"([^"]+)".*?"name":\s*"([^"]+)"', page_text, re.DOTALL)
            if json_ld_match:
                locality = json_ld_match.group(1)
                name = json_ld_match.group(2)
                if 'метро' in name or any(station in name for station in ['Академическая', 'Политехническая', 'Лесная']):
                    return name
            
            # 4. Ищем по известным станциям метро СПб
            stations = [
            ...
            for station in stations:
                if station.lower() in page_text_lower:
                    return station
    ...
    def extract_metro_time_improved(self):
        """Извлечь время до метро - СУПЕР УЛУЧШЕННЫЙ МЕТОД"""
        try:
            # Получаем весь текст страницы
            page_text = self.driver.page_source
            
            # 1. Ищем во всем HTML (самый широкий поиск)
            patterns = [
                r'\((\d+)\s*мин\.?\)',  # (10 мин)
                r'\((\d+)\s*минут\.?\)',  # (10 минут)
                r'(\d+)\s*мин\.?\s*(?:до метро|пешком|ходьбы|на транспорте)',  # 10 мин до метро
                r'до метро\s*(\d+)\s*мин\.?',  # до метро 10 мин
                r'пешком\s*(\d+)\s*мин\.?',  # пешком 10 мин
                r'(\d+)\s*минут\s*(?:до метро|пешком)',  # 10 минут до метро
                r'timeToMetro["\']\s*:\s*["\']?(\d+)',  # timeToMetro: "10"
                r'"time_to_metro"\s*:\s*(\d+)',  # "time_to_metro": 10
                r'"walkingTime"\s*:\s*["\']?(\d+)',  # "walkingTime": "10"
                r'(\d+)-минутная\s+ходьба',  # 10-минутная ходьба
                r'(\d+)\s*мин\.?\s*от\s*метро',  # 10 мин от метро
                r'(\d+)\s*минут\s*от\s*метро'  # 10 минут от метро
            ]
            ...
            # 2. Ищем в тексте элементов
            all_elements = self.driver.find_elements(By.XPATH, "//*")
            for elem in all_elements:
                try:
                    text = elem.text.strip()
                    if text and len(text) < 100:  # Не слишком длинные тексты
                        # Ищем паттерны времени
                        for pattern in [r'(\d+)\s*мин', r'(\d+)\s*минут']:
                            match = re.search(pattern, text, re.IGNORECASE)
                            if match:
                                time_val = match.group(1)
                                # Проверяем контекст - должно быть связано с метро
                                text_lower = text.lower()
                                if ('метро' in text_lower or 'пешком' in text_lower or 
                                    'ходьб' in text_lower or 'до станции' in text_lower):
                                    if time_val.isdigit() and 1 <= int(time_val) <= 120:
                                        return f"{time_val} мин"
                except:
                    continue
            
            # 3. Ищем в видимых текстовых блоках
            visible_selectors = [
                'p', 'div', 'span', 'li', '[class*="description"]', 
                '[class*="info"]', '[class*="feature"]'
            ]
            ...
            for selector in visible_selectors:
                try:
                    ...
                    for elem in elements:
                        text = elem.text.strip()
                        if 10 < len(text) < 500:  # Подходящая длина
                            # Ищем комбинацию "метро" и времени
    ...
    def show_stats(self):
        ...
        try:
            ...
            self.cursor.execute(f"""
                SELECT cian_id, metro_station, metro_time, building_type
                FROM {self.table_name} 
                WHERE metro_station IS NOT NULL AND metro_station != ''
            ...
            for ex in examples:
                ...
                print(f"  ID: {ex[0]}, {metro_info}{building_info}")
    ...
    def parse_offer(self, offer):
        ...
        try:
            ...
            data = {
                ...
                'rooms': self.extract_rooms(),
                'year_built': self.extract_year_built(),
                'building_type': self.extract_building_type_improved(),
                'property_type': self.extract_property_type(),
                'description': None,
                'seller_type': self.extract_seller_type(),
                'publication_date': datetime.now().strftime('%Y-%m-%d'),
                'is_active': True,
                'district': self.extract_district(),
                'metro_station': self.extract_metro_station_improved(),
                'metro_time': self.extract_metro_time_improved(),
                'last_checked': datetime.now()
            }
    ...
    def run(self):
        ...
        if use_db:
            ...
            print(f"   SELECT cian_id, metro_station, metro_time, building_type FROM {self.table_name} WHERE metro_station IS NOT NULL ORDER BY created_at DESC;")
            print(f"   SELECT * FROM {self.table_name} WHERE building_type IS NOT NULL LIMIT 5;")
