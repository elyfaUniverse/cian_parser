...
class CianParser:
    ...
    def parse_offer(self, offer):
        ...
        try:
            ...
            data = {
                ...
                'year_built': self.extract_year_built_improved(soup),  # Используем улучшенный метод
                'building_type': self.extract_building_type_improved(soup),  # Используем улучшенный метод
                'property_category': self.extract_property_category_improved(soup),  # Улучшенный метод для категории
                ...
                'metro_station': self.extract_metro_station_improved(soup),  # Улучшенный метод
                'metro_time': self.extract_metro_time_improved(soup),  # Улучшенный метод
    ...
    def extract_metro_time_improved(self, soup):
        ...
        try:
            ...
            # Сначала найдем все упоминания метро
            metro_positions = []
            for match in re.finditer(r'метро|станция', page_text, re.IGNORECASE):
                metro_positions.append(match.start())
            
            # Для каждого упоминания ищем время вблизи
            for pos in metro_positions:
                # Берем текст вокруг упоминания метро (100 символов в каждую сторону)
                start = max(0, pos - 100)
                ...
                for pattern in time_patterns:
                    ...
                    if match:
                        ...
                        if minutes.isdigit() and 1 <= int(minutes) <= 120:
                            return f"{minutes} мин"
            
            # 2. Ищем в специальных элементах
            metro_elements = soup.find_all(['div', 'span', 'a'], 
                                          class_=lambda x: x and x and any(word in str(x).lower() 
                                                                           for word in ['underground', 'metro', 'transport', 'time']))
            
            for elem in metro_elements:
                text = elem.get_text(strip=True)
                # Ищем время
                time_match = re.search(r'(\d+)\s*мин', text, re.IGNORECASE)
                if time_match:
            ...
            for script in scripts:
                if script.string:
                    # Ищем timeToMetro в различных форматах
                    time_patterns = [
                        r'"timeToMetro":\s*"(\d+)"',
                        r'"timeToMetro":\s*(\d+)',
                        r'"time_to_metro":\s*"(\d+)"',
            ...
            # 4. Общий поиск по странице
            patterns = [
                r'(\d+)\s*мин(?:ут)?\.?\s*(?:до метро|пешком|ходьбы|от метро)',
                r'до метро\s*(\d+)\s*мин(?:ут)?\.?',
                r'пешком\s*(\d+)\s*мин(?:ут)?\.?',
                r'(\d+)-минутная\s+ходьба',
            ]
            
            for pattern in patterns:
                matches = re.findall(pattern, page_text, re.IGNORECASE)
                for minutes in matches:
                    if str(minutes).isdigit() and 1 <= int(minutes) <= 120:
                        return f"{minutes} мин"
